...
resource "cassandra_user_level_table" "events" {
  depends_on = [
    ...
    cassandra_system_level_profile.read_heavy,
  ]

  keyspace                = "app_guardrail_e2e"
  table_name              = "events"
  if_not_exists           = true
  required_system_profile = "read_heavy"
